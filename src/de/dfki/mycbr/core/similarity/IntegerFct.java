...
/**
 * Calculates the similarity of IntegerAttribute objects. There are four modes
 ...
 * @author myCBR Team
 */
public class IntegerFct extends NumberFct {
	...
	public Similarity calculateSimilarity(Attribute a1, Attribute a2) throws Exception {
		...
		if (a1 instanceof SpecialAttribute
		    ...
		    result = prj
					.calculateSpecialSimilarity(a1, a2);
		} else if (a1 instanceof MultipleAttribute<?> && a2 instanceof MultipleAttribute<?>) {
			result = prj
			...
			if ((value1 instanceof IntegerAttribute)
					&& (value2 instanceof IntegerAttribute)) {
				IntegerAttribute att1 = (IntegerAttribute) value1;
				IntegerAttribute att2 = (IntegerAttribute) value2;
					...
					if ((d < 0 && distanceFunction == DistanceConfig.DIFFERENCE)
						...
						case SMOOTH_STEP_AT:
							result = Similarity.get(1 / (1 + Math
									.exp((-d + functionParameterL) * (100 / diff))));
							break;
						case CONSTANT:
	...
	/**
	 * Calculates the similarity of the given Integers. Calls
	 ...
	 * <li>POLINOMIAL WITH: the constant with which the function gets
	...
	public boolean setFunctionParameterR(double value) {
		...
		if (functionTypeR == NumberConfig.SMOOTH_STEP_AT
			...
			if (distanceFunction == DistanceConfig.DIFFERENCE && value > 0 && value <= diff) {
				functionParameterR = value;
				updated = true;
			} else if (distanceFunction == DistanceConfig.QUOTIENT && value > 1 && value <= max / min) {
				functionParameterR = value;
				updated = true;
			}
	...
	public boolean setFunctionParameterL(double value) {
		...
		if (functionTypeL == NumberConfig.SMOOTH_STEP_AT
			...
			if (distanceFunction == DistanceConfig.DIFFERENCE && value >= -diff) {
				...
				functionParameterL = value;
				updated = true;
			} else if (distanceFunction == DistanceConfig.QUOTIENT && value < 1 && value >= min / max) {
				functionParameterL = value;
				updated = true;
			}
	 ...
	 * Updates function parameter if necessary.
	 * Be aware to set the function type before setting the parameter, because the parameter depends
	 * on the current function type.
	 * 
	 * @param type
	...
	public void clone(AttributeDesc descNEW, boolean active) {
		if (descNEW instanceof IntegerDesc && !name.equals(Project.DEFAULT_FCT_NAME)) {
			IntegerFct f = ((IntegerDesc)descNEW).addIntegerFct(name, active);
	 ...
	 */
	public boolean fitsDistance(int x) {
