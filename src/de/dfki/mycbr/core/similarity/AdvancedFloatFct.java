...
public class AdvancedFloatFct extends NumberFct {
	...
	public Similarity calculateSimilarity(Attribute a1, Attribute a2) throws Exception {
		...
		if (a1 instanceof SpecialAttribute || a2 instanceof SpecialAttribute) {
			...
			FloatAttribute q = (FloatAttribute)a1;
			FloatAttribute c = (FloatAttribute)a2;
	...
	public Similarity calculateSimilarity(float q, float c) throws Exception {
	...
	public void clone(AttributeDesc descNEW, boolean active) {
		if (descNEW instanceof FloatDesc && !name.equals(Project.DEFAULT_FCT_NAME)) {
			AdvancedFloatFct f = ((FloatDesc)descNEW).addAdvancedFloatFct(name, active);
